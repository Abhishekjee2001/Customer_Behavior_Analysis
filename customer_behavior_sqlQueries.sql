...
select item_purchased, round(avg(review_rating)) as "Average product Rating" from customer
...
-- 5.Do subscribed customers spend more? compare  average spend  and total revenue between subscribers and non-subscriber?
...
round(sum(case when discount_applied ='Yes' then 1 else 0 end)/count(*)*100) as discount_rate
from customer
group by item_purchased
